...
class IntelligentCLIAgent:
    ...
    def _get_system_prompt(
        ...
        return dedent(f"""\
            ...
            ## 可用工具：
            - **today_date**: 获取当天日期
            - **cmds_list**: 列出单个网站的所有可用命令
            - **cmd_help**: 获取单个网站的单个命令详细帮助
            - **cmd_exec**: 执行完整命令字符串
            ...
            2，
            如果用户提到类似'今天'，'明天'等不确定的日期，则调用 today_date()
            ...
            ## 禁止：
            - 禁止编造数据: 必须调用工具取得真实数据
            - 禁止猜测指令参数: 必须先调用 cmds_list() 确认支持的命令，再调用 cmd_help() 获取命令参数
            - 禁止用相同的参数重复调用工具: 最多调用3次
            - 如果 cmd_exec() 返回正常数据: 禁止用相同的参数继续调用
            - 如果 cmd_exec() 返回(出错|超时|没有数据)并且尝试达到3次: 停止工具调用并提示用户
